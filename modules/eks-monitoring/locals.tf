...
locals {
  # Dashboard base URL — constructed from repo, ref, and path
  dashboard_base_url = "https://raw.githubusercontent.com/${var.dashboard_git_repo}/${var.dashboard_git_ref}"

  dashboard_path = var.dashboard_git_path != "" ? var.dashboard_git_path : (
    local.is_cloudwatch_otlp ? "artifacts/grafana-dashboards/eks/cloudwatch-otlp" : "artifacts/grafana-dashboards/eks/infrastructure"
  )

  # Dashboard file lists per profile
  cw_dashboard_files = {
    containers      = "containers.json"
    gpu-fleet       = "gpu-fleet.json"
    nodes           = "nodes.json"
    unified-service = "unified-service.json"
  }

  amp_dashboard_files = {
    cluster             = "cluster.json"
    kubelet             = "kubelet.json"
    namespace-workloads = "namespace-workloads.json"
    node-exporter       = "nodeexporter-nodes.json"
    nodes               = "nodes.json"
    workloads           = "workloads.json"
  }

  default_dashboard_files = local.is_cloudwatch_otlp ? local.cw_dashboard_files : local.amp_dashboard_files

  default_dashboard_sources = {
    for k, file in local.default_dashboard_files :
    k => "${local.dashboard_base_url}/${local.dashboard_path}/${file}"
  }
