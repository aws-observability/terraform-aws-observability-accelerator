# Removed provider as you can't have a provider defined here when you want to use count for this module in the root module because, a provider makes this a legacy module.. 
# provider "aws" {
...
resource "aws_lambda_function" "observability_accelerator_lambda" {
  ...
  memory_size   = 128
  timeout       = 180 
  # filename      = "../../modules/eks-key-rotation/lambda_function.zip"
...
resource "aws_scheduler_schedule" "eventbridge_scheduler" {
  ...
  target {
    ...
    input = jsonencode({
      ...
      workspaceid  = var.managed_grafana_workspace_id # changed this use env variable
    })
  }
}
